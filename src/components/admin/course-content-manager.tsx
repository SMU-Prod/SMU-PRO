    ...
    <div className="space-y-4">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-4 text-sm text-gray-500">
          ...
          <span className="text-[11px] text-gray-400 hidden sm:inline">Arraste pelo ⠿ para reordenar</span>
        </div>
      ...
      {modules.length === 0 && !addingModule && (
        <div className="flex flex-col items-center justify-center py-16 text-center border-2 border-dashed border-gray-200 rounded-xl">
          ...
          <p className="text-gray-900 font-medium mb-1">Nenhum módulo ainda</p>
          <p className="text-sm text-gray-500 mb-4">Crie o primeiro módulo para começar.</p>
...
function SortableModule({ mod, modIdx, expanded, editingModule, addingLesson, editingLesson, quizLesson, moduleForm, lessonForm, loading, sensors, onToggle, onEditModule, onCancelEditModule, onUpdateModule, onDeleteModule, onCreateLesson, onUpdateLesson, onAutoSaveLesson, onUpdateLessonState, onDeleteLesson, onDuplicateLesson, onMoveLesson, allModules, onStartAddLesson, onCancelAddLesson, onStartEditLesson, onCancelEditLesson, onToggleQuiz, onLessonDragEnd }: any) {
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <Card className={cn("overflow-hidden", isDragging && "shadow-lg ring-2 ring-[#6C3CE1]/20")}>
        {/* Module header */}
        <div
        ...
        >
          ...
          <div
            {...attributes}
            {...listeners}
            className="cursor-grab active:cursor-grabbing p-0.5 rounded text-gray-300 hover:text-gray-500 hover:bg-gray-100 transition-colors shrink-0"
          ...
          <div className="flex-1 min-w-0">
            {editingModule === mod.id ? (
              ...
              <p className="font-medium text-sm text-gray-900 truncate">{mod.titulo}</p>
            )}
          ...
          <ChevronDown size={16} className={cn("text-gray-400 transition-transform shrink-0", expanded && "rotate-180")} />
        </div>
        ...
        {expanded && (
          <div className="border-t border-gray-100">
            ...
            {addingLesson === mod.id ? (
              ...
              <button
                className="w-full flex items-center gap-2 px-4 py-2.5 text-sm text-gray-400 hover:text-[#6C3CE1] hover:bg-purple-50/50 transition-colors"
...
function SortableLesson({ lesson, lessonIdx, editingLesson, quizLesson, lessonForm, loading, onStartEdit, onCancelEdit, onUpdate, onAutoSave, onDelete, onDuplicate, onToggleQuiz, onLessonStateChange, onMoveToModule, allModules, currentModuleId }: any) {
  ...
  return (
    <div ref={setNodeRef} style={style} className={cn("group", isDragging && "z-10 relative")}>
      <div className="flex items-center gap-3 px-4 py-2.5 hover:bg-gray-50 transition-colors border-b border-gray-100/80">
        {/* Drag handle */}
        <div
          {...attributes}
          {...listeners}
          className="cursor-grab active:cursor-grabbing p-0.5 rounded text-gray-200 hover:text-gray-400 hover:bg-gray-100 transition-colors shrink-0"
        >
          <GripVertical size={14} />
        </div>

        <span className="text-xs text-gray-400 w-4 shrink-0">{lessonIdx + 1}</span>
        <LessonTypeIcon tipo={lesson.tipo} />

        <div className="flex-1 min-w-0">
          <p className="text-sm text-gray-900 truncate">{lesson.titulo}</p>
          <div className="flex items-center gap-2 mt-0.5">
            <span className="text-xs text-gray-400">{formatMinutes(lesson.duracao_min)}</span>
        ...
        <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
          <Button
            ...
            className={cn("h-7 w-7", quizLesson === lesson.id && "bg-purple-50 text-[#6C3CE1]")}
          ...
          {allModules && allModules.length > 1 && (
            <div className="relative">
              ...
              {showMoveMenu && (
                <div className="absolute right-0 top-8 z-50 w-48 rounded-xl border border-gray-200 bg-white shadow-lg py-1">
                  <p className="px-3 py-1.5 text-[10px] text-gray-400 uppercase tracking-wide font-semibold">Mover para</p>
    ...
    >
      <div className="grid grid-cols-2 gap-3">
        ...
        <div>
          ...
          <select
            {...form.register("tipo")}
            className="w-full h-8 rounded-lg border border-gray-200 bg-white px-2 text-xs text-gray-900 focus:outline-none focus:ring-2 focus:ring-[#6C3CE1]"
          >
      ...
      {(tipo === "video" || tipo === "misto") && (
        <div>
          <label className="block text-xs text-gray-500 mb-1">
            YouTube ID <span className="text-[10px] text-gray-400">(ex: dQw4w9WgXcQ)</span>
          </label>
          <div className="flex gap-2">
            ...
            {form.watch("youtube_id") && (
              <a
                ...
                className="flex items-center gap-1 px-2 h-8 text-[10px] text-[#6C3CE1] border border-[#6C3CE1]/30 rounded-lg hover:bg-purple-50 transition-colors shrink-0"
              >
                ▶ Preview
              </a>
            )}
      ...
      {(tipo === "texto" || tipo === "misto") && (
        <div>
          <label className="block text-xs text-gray-500 mb-1">Conteúdo Rico</label>
          <RichTextEditor
      ...
      <div className="flex items-center gap-4">
        <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-700">
          <input type="checkbox" {...form.register("tem_quiz")} className="accent-[#6C3CE1]" />
          Tem Quiz
        </label>
        <label className="flex items-center gap-2 cursor-pointer text-xs text-gray-700">
          <input type="checkbox" {...form.register("preview_gratis")} className="accent-[#6C3CE1]" />
          Preview Gratuito
        </label>
      </div>
      ...
      <div className="flex items-center gap-2">
        ...
        {autoSaving && <span className="text-xs text-gray-400 ml-auto animate-pulse">Salvando...</span>}
...
function QuizBuilder({ lessonId, onClose, onQuizCreated, onQuizDeleted }: { lessonId: string; onClose: () => void; onQuizCreated?: () => void; onQuizDeleted?: () => void }) {
  ...
  return (
    <div className="border-t border-purple-100 bg-purple-50/40 px-4 py-4">
      <div className="flex items-center justify-between mb-4">
        <div className="flex items-center gap-2">
          <HelpCircle size={15} className="text-[#6C3CE1]" />
          <span className="text-sm font-semibold text-gray-900">Builder de Quiz</span>
      ...
      {loading && !loaded ? (
        ...
        <div className="text-center py-6">
          ...
          <p className="text-sm text-gray-500 mb-3">Nenhum quiz criado ainda para esta aula.</p>
          <Button size="sm" onClick={handleCreateQuiz} loading={loading}><Plus size={13} /> Criar Quiz</Button>
        </div>
      ) : (
        <div className="space-y-3">
          {editingSettings ? (
            <div className="bg-white rounded-lg border border-purple-200 px-3 py-3 space-y-3">
              <p className="text-xs font-semibold text-gray-700">Configurações do Quiz</p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                <div>
                  <label className="block text-[10px] text-gray-500 mb-1">Aprovação mínima (%)</label>
                  <input
                    ...
                    className="w-full h-7 text-xs border border-gray-200 rounded-lg px-2 focus:outline-none focus:border-[#6C3CE1]"
                  />
                </div>
                <div>
                  <label className="block text-[10px] text-gray-500 mb-1">Tentativas</label>
                  <input
                ...
                <div>
                  <label className="block text-[10px] text-gray-500 mb-1">Tempo limite (min)</label>
                  <input
            ...
            <div className="flex items-center gap-3 bg-white rounded-lg border border-gray-200 px-3 py-2 text-xs text-gray-500 flex-wrap">
              <Settings2 size={13} className="text-gray-400" />
              <span>Aprovação: <strong className="text-gray-900">{quiz.nivel_minimo_aprovacao}%</strong></span>
              <span>·</span>
              <span>Tentativas: <strong className="text-gray-900">{quiz.tentativas_permitidas}x</strong></span>
              <span>·</span>
              <span className={quiz.embaralhar_questoes ? "text-emerald-600" : "text-gray-400"}>
                {quiz.embaralhar_questoes ? "Embaralhar ✓" : "Ordem fixa"}
              </span>
              {quiz.tempo_limite_min && (
                <>
                  <span>·</span>
                  <span>Tempo: <strong className="text-gray-900">{quiz.tempo_limite_min} min</strong></span>
                </>
              )}
              <div className="ml-auto flex items-center gap-1">
                <Button variant="ghost" size="icon" className="h-6 w-6 text-gray-400 hover:text-gray-700" onClick={() => setEditingSettings(true)} title="Editar configurações">
                  <Edit2 size={11} />
                </Button>
          ...
          {addingQuestion ? (
            ...
            <button
              className="w-full flex items-center gap-2 px-3 py-2 text-sm text-gray-400 hover:text-[#6C3CE1] hover:bg-white border border-dashed border-gray-200 hover:border-purple-300 rounded-lg transition-colors"
...
function QuestionCard({ question, index, expanded, onToggle, onDelete, onUpdate, onAddOption, onSetCorrect, onDeleteOption, loading }: any) {
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-200 overflow-hidden">
      <div className="flex items-center gap-3 px-3 py-2.5 cursor-pointer hover:bg-gray-50 select-none" onClick={onToggle}>
        <span className="flex h-5 w-5 items-center justify-center rounded-full bg-purple-100 text-[10px] font-bold text-purple-700 shrink-0">{index}</span>
        <div className="flex items-center gap-1.5 flex-1 min-w-0">
          {isTrueFalse && <Badge variant="secondary" className="text-[9px] px-1 py-0 shrink-0">V/F</Badge>}
          <p className="text-sm text-gray-900 truncate">{question.texto}</p>
        </div>
        <div className="flex items-center gap-2 shrink-0">
          {hasCorrect
            ? <CheckCircle2 size={13} className="text-emerald-500" />
            : <span title="Nenhuma resposta correta marcada"><Circle size={13} className="text-amber-400" /></span>}
          <span className="text-[10px] text-gray-400">{question.quiz_options?.length ?? 0} opções</span>
      ...
      {expanded && (
        <div className="border-t border-gray-100 px-3 py-3 space-y-2">
          ...
          {editingText ? (
            <div className="space-y-2 mb-3 p-2 rounded-lg bg-gray-50 border border-gray-200">
              <textarea
                className="w-full text-sm border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#6C3CE1] resize-none"
                rows={2}
                value={editText}
              ...
              />
              <input
                className="w-full text-xs border border-gray-200 rounded-lg px-2 py-1.5 focus:outline-none focus:border-[#6C3CE1]"
            ...
            <button
              className="text-[10px] text-[#6C3CE1] hover:underline mb-1"
          ...
          {(question.quiz_options ?? []).map((opt: any) => (
            <div key={opt.id} className="flex items-center gap-2 group">
              <button
                className={cn("flex h-5 w-5 items-center justify-center rounded-full border-2 shrink-0 transition-colors", opt.correta ? "border-emerald-500 bg-emerald-500" : "border-gray-300 hover:border-emerald-400")}
          ...
          {!isTrueFalse && (
            <>
              <div className="flex items-center gap-2 pt-1">
                <button
                  className={cn("flex h-5 w-5 items-center justify-center rounded-full border-2 shrink-0 transition-colors", isCorrect ? "border-emerald-500 bg-emerald-500" : "border-gray-300 hover:border-emerald-400")}
                ...
                <input
                  className="flex-1 text-sm border border-gray-200 rounded-lg px-2 py-1 focus:outline-none focus:border-[#6C3CE1]"
                  placeholder="Nova opção..."
                  value={newOption}
                  onChange={(e) => setNewOption(e.target.value)}
                  onKeyDown={(e) => { if (e.key === "Enter") { e.preventDefault(); onAddOption(newOption, isCorrect); setNewOption(""); setIsCorrect(false); } }}
                />
                <Button size="sm" variant="ghost" className="h-7 px-2 text-[#6C3CE1]" loading={loading} onClick={() => { onAddOption(newOption, isCorrect); setNewOption(""); setIsCorrect(false); }}>
                  <Plus size={12} />
                </Button>
              </div>
              <p className="text-[10px] text-gray-400">Clique no círculo para marcar a resposta correta. Enter para adicionar.</p>
            </>
          )}

          {isTrueFalse && (
            <p className="text-[10px] text-gray-400">Clique no círculo para definir se a afirmação é verdadeira ou falsa.</p>
          )}
        </div>
      )}
    </div>
...
function AddQuestionForm({ onSave, onCancel, loading }: { onSave: (texto: string, tipo: "multiple_choice" | "true_false") => void; onCancel: () => void; loading: boolean }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-purple-200 px-3 py-3 space-y-2">
        ...
        <div className="flex gap-1">
          <button
            className={cn(
              ...
              tipo === "multiple_choice" ? "bg-[#6C3CE1] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
            )}
          ...
          <button
            className={cn(
              "px-2.5 py-1 rounded-lg text-xs font-medium transition-colors",
              tipo === "true_false" ? "bg-[#6C3CE1] text-white" : "bg-gray-100 text-gray-500 hover:bg-gray-200"
            )}
      ...
      />
      {tipo === "true_false" && (
        <p className="text-[10px] text-gray-400">As opções &quot;Verdadeiro&quot; e &quot;Falso&quot; serão criadas automaticamente.</p>
      )}
      <div className="flex gap-2">
