        ...
        <div className="relative">
          ...
          {enrollOpen && (
            <>
              ...
              <div className="absolute right-0 top-8 z-50 w-64 rounded-xl border border-gray-200 bg-white shadow-xl p-2 space-y-0.5">
                <div className="flex items-center justify-between px-2 py-1.5 mb-1">
                  <p className="text-xs font-semibold text-gray-700">Matricular em curso</p>
                ...
                {courses.map((c: any) => (
                  <button
                    ...
                    className="w-full text-left px-2 py-2 rounded-lg hover:bg-gray-50 transition-colors flex items-center gap-2 text-xs"
                  >
                    {enrolling === c.id ? (
                      <span className="h-4 w-4 rounded-full border-2 border-[#6C3CE1] border-t-transparent animate-spin shrink-0" />
                    ) : (
                      <span className="h-4 w-4 flex items-center justify-center shrink-0 text-gray-300">
                        <BookPlus size={11} />
                      </span>
                    )}
      ...
      <DropdownMenu.Root>
        ...
        <DropdownMenu.Portal>
          <DropdownMenu.Content
            className="z-50 min-w-[180px] rounded-xl border border-gray-200 bg-white p-1.5 shadow-xl animate-fade-in"
            sideOffset={4}
            align="end"
          >
            <DropdownMenu.Label className="px-3 py-1.5 text-[10px] text-gray-400 uppercase tracking-widest font-semibold">
              Alterar Role
            </DropdownMenu.Label>
            {roles.map((r) => (
              <DropdownMenu.Item
                key={r.value}
                className="flex items-center justify-between gap-2 px-3 py-2 text-xs text-gray-700 cursor-pointer rounded-lg hover:bg-purple-50 hover:text-[#6C3CE1] outline-none transition-colors"
                onClick={() => handleRoleChange(r.value)}
              >
                <span className={user.role === r.value ? "text-[#6C3CE1] font-semibold" : ""}>
                  {r.label}
                </span>
                {user.role === r.value && <Check size={12} className="text-[#6C3CE1]" />}
              </DropdownMenu.Item>
            ))}
