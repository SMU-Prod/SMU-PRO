...
const STATIC_COMMANDS: CommandItem[] = [
  ...
  { id: "cursos", label: "Cursos", sublabel: "Gerenciar todos os cursos", icon: Layers, iconColor: "text-purple-500", href: "/admin/cursos", group: "Navegação" },
  ...
  { id: "configuracoes", label: "Configurações", sublabel: "Configurações da plataforma", icon: Settings, iconColor: "text-gray-500", href: "/admin/configuracoes", group: "Navegação" },
  // Ações rápidas
  { id: "novo-curso", label: "Criar Novo Curso", sublabel: "Abre o wizard de criação", icon: Plus, iconColor: "text-[#6C3CE1]", href: "/admin/cursos/novo", group: "Ações Rápidas" },
];
  ...
  return (
    <>
      ...
      <button
        ...
        className="hidden md:flex items-center gap-2 h-8 px-3 rounded-lg border border-gray-200 bg-white text-sm text-gray-400 hover:border-purple-300 hover:text-gray-600 transition-colors"
        title="Busca rápida (Ctrl+K)"
      >
        <Search size={13} />
        <span>Buscar...</span>
        <div className="ml-2 flex items-center gap-0.5 text-[10px] text-gray-300 font-mono">
          <Command size={10} />
          <span>K</span>
        </div>
      </button>
      ...
      <Dialog.Root open={open} onOpenChange={setOpen}>
        <Dialog.Portal>
          ...
          <Dialog.Content
            className="fixed left-1/2 top-[15%] z-50 w-full max-w-lg -translate-x-1/2 rounded-2xl bg-white shadow-2xl ring-1 ring-gray-200 overflow-hidden animate-in fade-in-0 zoom-in-95"
          ...
          >
            ...
            <div className="flex items-center gap-3 border-b border-gray-100 px-4 py-3">
              ...
              />
              <kbd className="text-[10px] text-gray-400 font-mono border border-gray-200 rounded px-1.5 py-0.5">ESC</kbd>
            </div>
            ...
            <div className="max-h-80 overflow-y-auto py-2">
              {filtered.length === 0 ? (
                <div className="py-8 text-center text-sm text-gray-400">
                  Nenhum resultado para "<span className="text-gray-700">{query}</span>"
                </div>
              ) : (
                  ...
                  <div key={group}>
                    ...
                    {items.map((item) => {
                      ...
                      return (
                        ...
                        >
                          <div className={cn("flex h-8 w-8 items-center justify-center rounded-lg bg-gray-100 shrink-0", isActive && "bg-purple-100")}>
                            <Icon size={15} className={item.iconColor ?? "text-gray-500"} />
                          </div>
                          <div className="flex-1 min-w-0">
                            <p className="text-sm font-medium text-gray-900 truncate">{item.label}</p>
                            {item.sublabel && <p className="text-xs text-gray-400 truncate">{item.sublabel}</p>}
                          </div>
                          {isActive && <ArrowRight size={13} className="text-[#6C3CE1] shrink-0" />}
                        </button>
                      );
                    })}
                  </div>
            ...
            <div className="border-t border-gray-100 px-4 py-2 flex items-center gap-4 text-[10px] text-gray-400">
              ...
              <span><kbd className="font-mono border border-gray-200 rounded px-1">Enter</kbd> abrir</span>
              <span><kbd className="font-mono border border-gray-200 rounded px-1">Esc</kbd> fechar</span>
            </div>
          </Dialog.Content>
        </Dialog.Portal>
      </Dialog.Root>
